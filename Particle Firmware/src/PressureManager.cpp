...
void pressureManager::update(){
  if(this->isCharging) {
    uint32_t currentPressure = this->readPressure();
...
uint32_t pressureManager::updateTargetPressure(uint32_t _targetPressure){
...
uint32_t pressureManager::readPressure(){
  uint32_t pressureRaw = analogRead(PRESS_SNS_PIN)*3300/4095; // conver 12bit ADC to millivolts
